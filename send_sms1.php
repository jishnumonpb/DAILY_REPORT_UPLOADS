<?php
	...
	if(isset($_POST["save"]))
	{
		...
		// $l_date=$_POST["l_date"];
        // $brate=$_POST["brate"];
		//$suc =mysqli_query($conn, "insert into tbl_send_sms(con_message) values($id,'$sms')");
		echo "update tbl_send_sms set con_message='$sms' where uid=$id";
		$suc =mysqli_query($conn, "update tbl_send_sms set con_message='$sms' where uid=$id");// or die (mysqli_error());

			//$qury="INSERT INTO `tbl_addauction`(`add_date`,`bp`,`l_date`,`brate`) VALUES ('$add_date','$bp','$l_date','$brate')";
			//$obj=mysqli_query($conn,$qury);

		//echo "insert into tbl_send_sms(con_message) values($id,'$sms')";
